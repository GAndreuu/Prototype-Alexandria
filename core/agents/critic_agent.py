...
class RiskLevel(Enum):
    """Níveis de risco para hipóteses."""
    BAIXO = "baixo"
    MEDIO = "medio" 
    ALTO = "alto"
    CRITICO = "critico"
...
class TruthScore(Enum):
    """Categorias de pontuação de veracidade."""
    FACTUAL = "factual"
    PLAUSIVEL = "plausivel"
...
class CriticalAssessment:
    ...
    evidence_quality: float  # Qualidade das evidências
    contradiction_strength: float  # Força das contradições
    reasoning_coherence: float  # Coerência do raciocínio
...
class CriticAgent:
    """
    V10 - Agente Crítico Sênior
    
    Responsabilidades:
    1. Avaliar hipóteses geradas pelo V9 usando Gemini
    2. Calcular Pontuações de Risco baseadas em evidências
    3. Fornecer feedback para auto-regulagem do sistema
    4. Detectar padrões de bias e contradições
    5. Sugerir ajustes nos parâmetros do TinyLlama e Manifold
    """
    ...
    def __init__(self, 
        ...
        """
        Inicializa o Agente Crítico Sênior.
        
        Args:
            gemini_api_key: Chave da API do Gemini
            sfs_path: Caminho para o Semantic File System
            temperature_history: Histórico de temperaturas para auto-regulagem
            risk_tolerance: Tolerância de risco do sistema (0.0 a 1.0)
        """
        ...
        logger.info("V10 - Agente Crítico Sênior inicializado")
    ...
    async def assess_hypothesis(self, 
        ...
        """
        Avalia uma hipótese gerada pelo V9 usando Gemini e evidências do SFS.
        
        Args:
            hypothesis: Hipótese do V9 AbductionEngine
            supporting_evidence: Evidências que suportam a hipótese
            contradicting_evidence: Evidências que contradizem a hipótese
            
        Returns:
            CriticalAssessment com pontuação de risco e análise detalhada
        """
        ...
        try:
            ...
            # 2. Chamar Gemini para análise crítica
            ...
            # 4. Atualizar histórico e métricas
            self._update_assessment_history(assessment)
            self._update_system_metrics(assessment)
            
            # 5. Detectar padrões de bias
            self._detect_bias_patterns(assessment)
            
            logger.info(f"Hipótese {hypothesis.get('id', 'unknown')} avaliada com score {assessment.truth_score:.3f}")
            return assessment
            
        except Exception as e:
            logger.error(f"Erro ao avaliar hipótese: {str(e)}")
            # Retornar avaliação conservadora em caso de erro
            return self._create_fallback_assessment(hypothesis)

    async def _call_gemini_critic(self, prompt: str) -> Dict[str, Any]:
        """
        Chama o Gemini para análise crítica da hipótese.
        ...
        if not self.gemini_api_key:
            logger.warning("Gemini API key não configurada, usando fallback local")
            return self._simulate_gemini_response(prompt)
        
        try:
            # Implementação real da chamada para Gemini API
    ...
    def _simulate_gemini_response(self, prompt: str) -> Dict[str, Any]:
        """
        Simula resposta do Gemini para demonstração com análise científica aprimorada.
        Em produção, substitua pela chamada real à API.
        """
        # Análise científica baseada em padrões
        score = 0.5  # Score neutro por padrão
        
        # Indicadores científicos positivos
        positive_indicators = [
            "fundamental", "prerequisito", "base", "fundamentação", "essencial",
            "matemática", "física", "conceito", "teoria", "evidência",
            "confirmado", "validado", "estabelecido", "conhecido"
        ]
        
        negative_indicators = [
            "contraditório", "refutar", "incorreto", "falso", "errado",
            "inconsistente", "inválido", "especulativo", "questionável"
        ]
        
        strong_evidence_indicators = [
            "evidência forte", "estudos mostram", "pesquisa demonstra",
        ...
        # Bonus especial para conexões matemáticas-físicas conhecidas
        if "matemática" in prompt_lower and "física" in prompt_lower:
            score += 0.20
        if "álgebra" in prompt_lower and "quântica" in prompt_lower:
            score += 0.25
        ...
        return {
            ...
            "reasoning": f"Análise científica baseada em padrões: {category} (score: {score:.2f})",
            ...
            "coherence_score": min(1.0, score + 0.1)  # Coerência ligeiramente melhor que score
        }
    
    def _extract_supporting_facts(self, prompt: str) -> List[str]:
        """Extrai fatos de suporte do prompt"""
        facts = []
        if "matemática" in prompt and "física" in prompt:
            facts.append("Fundamentos matemáticos são base para física avançada")
        if "álgebra" in prompt and "quântica" in prompt:
            facts.append("Espaços vetoriais são essenciais na mecânica quântica")
        if "evidência" in prompt:
            facts.append("Evidências empíricas suportam a relação causal")
        if not facts:
            facts = ["Evidência conceitual 1", "Padrão científico 2"]
        return facts
    
    def _extract_contradicting_facts(self, prompt: str) -> List[str]:
        """Extrai fatos contraditórios do prompt"""
        facts = []
        if "contraditório" in prompt or "refutar" in prompt:
            facts.append("Evidência contrária encontrada")
        if "questionável" in prompt:
            facts.append("Falta de evidência conclusiva")
        return facts

    def _parse_gemini_score(self, response_text: str) -> Optional[float]:
        """
        Extrai score numérico da resposta do Gemini
        """
        import re
        
        # Procurar padrões como "score: 0.7", "truth score: 0.7", etc.
        patterns = [
            r'score\s*[=:]\s*(\d*\.?\d+)',
            r'truth\s+score\s*[=:]\s*(\d*\.?\d+)',
            r'(\d*\.?\d+)\s*[/=]\s*1',
            r'(\d*\.?\d+)\s*por\s*cento',
            r'nível\s+de\s+confiança\s*[=:]\s*(\d*\.?\d+)'
        ]
        ...
        for pattern in patterns:
            ...
            if match:
                try:
                    ...
                    # Normalizar se está em porcentagem
                    if score > 1:
                        score = score / 100
        ...
        # Se não conseguiu extrair, retornar None para usar fallback
        return None
    ...
    def _prepare_criticism_prompt(self, 
        ...
        """
        Prepara prompt estruturado para análise crítica pelo Gemini.
        """
        prompt = f"""
        ANÁLISE CRÍTICA DE HIPÓTESE - PROTOTYPE ALEXANDRIA
        
        HIPÓTESE A AVALIAR:
        - ID: {hypothesis.get('id', 'N/A')}
        - Fonte: {hypothesis.get('source_cluster', 'N/A')}
        - Alvo: {hypothesis.get('target_cluster', 'N/A')}
        - Confiança: {hypothesis.get('confidence_score', 'N/A')}
        ...
        INSTRUÇÕES:
        1. Avalie a veracidade da hipótese (0.0 a 1.0)
        2. Identifique riscos potenciais
        3. Liste fatos que suportam a hipótese
        4. Liste fatos que contradizem a hipótese
    ...
    def _format_evidence_list(self, evidence_list: List[Dict]) -> str:
        """Formata lista de evidências para o prompt."""
        if not evidence_list:
            return "Nenhuma evidência disponível."
            
        formatted = []
        for i, evidence in enumerate(evidence_list[:5], 1):  # Limitar a 5 evidências
    ...
    def _parse_gemini_response(self, 
        ...
        # Calcular qualidade das evidências
    ...
    def _calculate_evidence_quality(self, 
        ...
        # Calcular score médio das evidências
        ...
        # Bonus por quantidade de evidências
    ...
    def _identify_evidence_gaps(self, 
        ...
        """Identifica lacunas nas evidências."""
        gaps = []
        
        if len(supporting_evidence) < 3:
            gaps.append("Evidências de suporte insuficientes")
        
        if len(contradicting_evidence) > len(supporting_evidence):
            gaps.append("Evidências contraditórias superam as de suporte")
        
        if not supporting_evidence:
            gaps.append("Nenhuma evidência de suporte encontrada")
    ...
    def _generate_adjustment_suggestions(self, 
        ...
        """Gera sugestões de ajuste baseadas na avaliação."""
        suggestions = []
        
        if truth_score < 0.5:
            suggestions.append("Considerar gerar hipóteses mais conservadoras")
            suggestions.append("Revisar fontes de treinamento do V9")
        
        if evidence_quality < 0.6:
            suggestions.append("Expandir base de evidências no SFS")
            suggestions.append("Melhorar indexação semântica")
        
        if risk_level == RiskLevel.CRITICO:
            suggestions.append("Implementar validação adicional obrigatória")
            suggestions.append("Reduzir temperatura criativa do TinyLlama")
        
        if risk_level == RiskLevel.ALTO:
            suggestions.append("Adicionar camada de revisão manual")
        
        return suggestions

    def _create_fallback_assessment(self, hypothesis: Dict[str, Any]) -> CriticalAssessment:
        """Cria avaliação conservadora em caso de erro."""
        return CriticalAssessment(
            hypothesis_id=hypothesis.get("id", "unknown"),
            ...
            supporting_facts=[],
            contradicting_facts=[],
            gaps_in_evidence=["Falha na análise crítica automática"],
            recommendation="revisar",
            suggested_adjustments=["Verificar configuração do Gemini", "Investigar logs do sistema"],
            assessed_at=datetime.now()
        )

    def _update_assessment_history(self, assessment: CriticalAssessment):
        """Atualiza histórico de avaliações."""
        self.assessment_history.append(assessment)
        
        # Manter apenas as últimas 1000 avaliações para performance
    ...
    def _update_system_metrics(self, assessment: CriticalAssessment):
        ...
        # Atualizar média móvel do truth score
    ...
    def _detect_bias_patterns(self, assessment: CriticalAssessment):
        """Detecta padrões de bias no sistema."""
        recent_assessments = self.assessment_history[-50:]  # Últimas 50 avaliações
        
        if not recent_assessments:
            return
        
        # Confirmation bias: alta taxa de aprovação
        ...
        # Availability bias: excesso de evidências de um tipo
    ...
    async def get_system_feedback(self) -> SystemFeedback:
        ...
        Returns:
            SystemFeedback com métricas e recomendações
        """
        recent_assessments = self.assessment_history[-100:]  # Últimas 100 avaliações
        ...
        # Identificar contradições comuns
    ...
    def _find_common_contradictions(self, assessments: List[CriticalAssessment]) -> List[str]:
        """Encontra contradições comuns nas avaliações."""
        contradiction_count = {}
        ...
        # Retornar contradições mais comuns
    ...
    def _calculate_parameter_adjustments(self, 
        ...
        """
        Calcula ajustes recomendados para parâmetros do sistema.
        ...
        # Ajustar temperatura baseada na taxa de aprovação
        ...
        # Ajustar variância baseada em bias
        total_bias = sum(bias_indicators.values())
        if total_bias > 0.6:
            # Alto bias detectado, aumentar variância para diversidade
            var_adjustment = 0.05
    ...
    def apply_system_adjustments(self, temperature_adjustment: float, variance_adjustment: float):
        """
        Aplica ajustes automáticos aos parâmetros do sistema.
        
        Args:
            temperature_adjustment: Ajuste de temperatura (-1.0 a 1.0)
            variance_adjustment: Ajuste de variância (-1.0 a 1.0)
        """
        # Registrar histórico de temperaturas
    ...
    def _generate_markdown_report(self) -> str:
        ...
        report = f"""# Relatório do Agente Crítico Sênior - V10
        
## Métricas do Sistema
- **Total de Avaliações**: {len(self.assessment_history)}
- **Taxa de Aprovação**: {(self.system_metrics['approved_hypotheses'] / max(self.system_metrics['total_assessments'], 1) * 100):.1f}%
- **Score Médio de Veracidade**: {self.system_metrics['average_truth_score']:.3f}
...
"""
        ...
        for assessment in self.assessment_history[-5:]:
            report += f"### Hipótese {assessment.hypothesis_id}\n"
            report += f"- **Score de Veracidade**: {assessment.truth_score:.3f}\n"
            report += f"- **Nível de Risco**: {assessment.risk_level.value}\n"
            report += f"- **Recomendação**: {assessment.recommendation}\n"
